// Creates the 5 production templates on the PRODUCTION WABA (not the test one).
// Usage: node scripts/create-production-templates.cjs [--status-only]
import 'dotenv/config';

const API = 'https://graph.facebook.com/v21.0';
const token = process.env.WHATSAPP_ACCESS_TOKEN;
const PROD_WABA = '2035908973733316'; // Production WABA "Zostel"
...
const TEMPLATES = [
  ...
  },
  {
    ...
    body:
      ...
      'Do a quick sweep for chargers, chargers cables and that one sock hiding under the bed \u{1F604}\n\n' +
...
async function statusOnly() {
  ...
  const ours = res.body.data.filter((t) => names.has(t.name));
  if (ours.length === 0) {
    console.log('  (no matching templates found on production WABA)');
  }
  for (const t of ours) {
    console.log(
