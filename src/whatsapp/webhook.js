...
// Twilio WhatsApp webhook payloads (application/x-www-form-urlencoded):
...
export async function handleTwilioEvent(params) {
  const event = classifyTwilioEvent(params);
  if (event.kind === 'status') {
    await applyStatus(event.messageSid, event.messageStatus, params.ErrorMessage ?? null);
    return event;
  }
  if (event.kind === 'inbound') {
    await ingestInboundText({
      ...
      text: params.Body ?? '',
      waMessageId: params.MessageSid ?? params.SmsSid,
    });
    return event;
  }
  return event;
...
async function applyStatus(messageSid, twilioStatus, errorMessage) {
  ...
  const msg = rows[0];
  if (!msg) {
    console.warn(`[webhook] status ${status} for unknown MessageSid ${messageSid}`);
    return;
  }
...
export async function ingestInboundText({ phone, profileName, text, waMessageId }) {
  ...
  if (existing.length > 0) {
    ...
    console.log(`[webhook] new guest created from inbound message: ${phone}`);
  }
